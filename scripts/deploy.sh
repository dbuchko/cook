...
#{
#  cf create-service -c '{ "git": { "uri": "https://github.com/dbuchko/cook-config", "label": "main"  } }' ${service_name} standard cook-config-server
#} &>/dev/null
...
cf push -p "$@" -f ../manifest.yml -b java_buildpack_offline
echo "Done!"
